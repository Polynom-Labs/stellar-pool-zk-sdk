...
import { BINDING_ZK_NONCE, SIX_BY_SIX_BINDING_ZK_NONCE } from './zk-layout';
...
function resolveZkInit(parsed: Record<string, string>): {
  ...
  if (explicit && (explicit === '6x6' || explicit === 'six' || explicit === '6')) {
...
Commands:
  ...
  withdraw                         Generate a withdrawal proof
    ...
    --application-id <dec>         Application id (decimal Fr); default: APPLICATION_ID env or 101
    --profile <2x2|6x6>            Circuit artifact profile (default: ZK_PROFILE / ZK_CONFIG_NONCE=7 → 6x6)
...
function requireParsedArg(parsed: Record<string, string>, name: string): string {
...
function requireSpendScalarDomain(parsed: Record<string, string>): SpendScalarDomain {
...
function parseCoinValueDecimal(label: string, raw: string | undefined, defaultStroops: bigint): string {
  if (raw === undefined) {
...
async function handleWithdraw(args: string[]) {
  ...
  const state: StateFile = JSON.parse(fs.readFileSync(parsed['state'], 'utf-8'));
  ...
  const applicationId = resolveApplicationIdFromCli(parsed);
  const audit = resolveTransactionAuditParams(applicationId, undefined, sdk.getLayout().nAuditSlots);
...
async function handleDepositProof(args: string[]) {
  ...
  const publicLegs: TransactionPublicLegParams = {
    ...
    publicDepositedAssets: [[deposit.asset[0], deposit.asset[1]] as [string, string]],
